...
corp_pre <- corpus_subset(corp_repec, year <= 2009)
...
tmod_lss <- textmodel_lss(dfmat_sent, seeds = seed,
                          terms =c("fiscal_multiplier", "fiscal_multipliers", "government_debt", 
                                   ...
                                   "multiplier*", "deficit_financed"), k = 300, cache = TRUE)


("fiscal multiplier*", "fiscal_multipliers", "government_debt", 
  "government spending","new keynesian", "new keynes", "new_keynesians",
  "spending multiplier", "spending multipliers", "keynesian multiplier", 
  "keynesian multipliers", "lagrange mutliplier", "lagrange multipliers",
  "state spending", "state spendings", "government spending", "government
               spendings", "public spending", "public spendings", "fiscal stimulus",
...
tmod_lss_after <- tmod_lss

save(tmod_lss_after, file ="tmod_lss_after.RData")


head(coef(tmod_lss_after), 50) # most positive words

tail(coef(tmod_lss_after), 20) # most negative words

### Typical negative words are visable. We see many negative words as negative
### but not few as positive as well

textplot_terms(tmod_lss_after, data_dictionary_LSD2015["negative"])



dfmat_doc <- dfm_group(dfmat_sent)
dat <- docvars(dfmat_doc)
dat$fit <- predict(tmod_lss_after, newdata = dfmat_doc)
