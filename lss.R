...
eco <- char_context(toks_sent, pattern =c("multiplier*", "keynes*", "countercyclical",
                                            "fiscal*", "spending*",
                                            "neo_keynes*", "government*",
                                            "fiscal deficit*",
                                          "crowding-in","new keyesian", "deficit*"), p = 0.05)
eco <- char_context(toks_sent, pattern =c("consolidation*", "discipline*", "prudence",
                                          "crowding*", "auster*", "orthodoxy", "ricard*",
                                          "crowding-out", "neoclassical"), p = 0.05)
eco <- char_context(toks_sent, pattern =c("fiscal_multiplier", "fiscal_multipliers", "government_debt", 
...
tmod_lss <- textmodel_lss(dfmat_sent, seeds = seed,
                          terms =c("multiplier", "debt", "fiscal space", 
                                   "keynes*", "countercyclical", "spending", "deficit*",
                                   "MPC", "HANK", "new keynes*", "deficit-financed"), k = 300, cache = TRUE)

tmod_lss <- textmodel_lss(dfmat_sent, seeds = seed,
                          terms =c("fiscal multiplier*", "fiscal multipliers", "government debt", 
                                   "government spending","new keynesian", "new keynes", "new keynesians",
                                   "spending multiplier", "spending multipliers", "keynesian multiplier", 
...
("fiscal multiplier*", "fiscal multipliers", "government debt", 
  "government spending","new keynesian", "new keynes", "new keynesians",
  "spending multiplier", "spending multipliers", "keynesian multiplier", 
  "keynesian multipliers", "lagrange mutliplier", "lagrange multipliers",
  "state spending", "state spendings", "government spending", "government
...
tmod_lss <- textmodel_lss(dfmat_sent, seeds = seed,
                          terms =c("consolidation*", "discipline*", "prudence",
                                   "crowding*", "auster*", "orthodoxy", "ricard*"
...
head(coef(tmod_lss), 50) # most positive words
...
plot(dat$date, dat$fit, col = rgb(0, 0, 0, 0.05), pch = 16, ylim = c(-0.3, 0.3),
