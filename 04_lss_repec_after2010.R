...
corp_repec <- corpus_subset(corp_repec, year > 2009) ### After
...
tmod_lss_after <- textmodel_lss(dfmat_sent, seeds = seed,
                          terms =c("budget_deficit", "budget_deficits","bailout", "bailouts", "crowding_in",
                                   "consumption_multiplier","consumption_multipliers", "countercyclical",
                                   "fiscal_deficit", "fiscal_deficits", "fiscal_multiplier",
                                   "fiscal_multipliers", "fiscal_stimulus", "fiscal_stimuli",
                                   "government_debt", "government_spending", "government_consumption",
                                   "government_intervention", "government_interventions",
                                   "public_spending", "new_keynesian", "neo-keynesian",
                                   "neo-keynesians", "keynes", "keynesian", "keynesians", "keynesianism",
                                   "spending_multiplier", "spending_multipliers", "keynesian_multiplier",
                                   "keynesian_multipliers", "state_spending", "state_spendings", "stimulus",
                                   "stimuli", "state_intervention", "state_interventions", "public_debt", 
                                   "policy_intervention", "policy_interventions", "deficit_spending",
                                   "regulation",
                                   "regulatory_policy", "regulatory_policies", "nationalization",
                                   "nationalizations", "nationalisation", "HANK", "fiscal policy",
                                   "fiscal policies", "inflation"), k = 300, cache = TRUE)





save(tmod_lss_after, file ="tmod_lss_after.RData")

rm(list=ls()[! ls() %in% c("dfmat_sent","seed")])


# Get Tmod LSS after classical dict ####
tmod_lss_after_nclassic <- textmodel_lss(dfmat_sent, seeds = seed,
...
save(tmod_lss_after_nclassic, file ="tmod_lss_after_nclassic.RData")
